...
| 요소 | 설명 |
|------|------|
| 헤더 | 월 네비게이션, 다크모드 토글, 로그아웃 |
| 티커 (`ticker-wrap`) | 공지사항 + 다가올 메모 좌우 스크롤 |
| 달력 그리드 (`cal-grid`) | 7열, 셀 클릭 시 상세 시트 열림 |
| 하단 바 (`bottom-bar`) | 반출/복구, 조 필터, 오늘/갱신 버튼 |
...
| 연차 | `연차:이름` | `연차:김철수` |
| 대체 | `대체:이름` | `대체:이영희` |
| 대휴 | `대휴:이름(YY-MM-DD)` | `대휴:김철수(25-06-15)` |
| 교육 | `교육:내용(참조)` | `교육:신입교육(2025-06)` |
| 출장 | `출장:내용(참조)` | `출장:서울(3일)` |
| 기타 | `기타:내용(참조)` | `기타:병가` |
...
| 항목 | 내용 | 영향도 |
|------|------|--------|
| JWT 토큰 갱신 없음 | 1시간 후 API 호출 실패 | 높음 |
...
| `SB_AUTH_EMAIL` | `starred3@naver.com` | 공용 로그인 이메일 |
